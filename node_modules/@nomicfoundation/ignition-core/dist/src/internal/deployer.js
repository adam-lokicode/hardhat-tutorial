...
class Deployer {
    ...
    async deploy(ignitionModule, deploymentParameters, accounts, defaultSender) {
        ...
        this._emitDeploymentStartEvent(ignitionModule.id, this._deploymentDir, isResumed, this._config.maxFeeBumps);
        ...
        (0, assertions_1.assertIgnitionInvariant)(contractStates.every((exState) => exState.type === execution_state_1.ExecutionSateType.DEPLOYMENT_EXECUTION_STATE ||
            exState.type === execution_state_1.ExecutionSateType.CONTRACT_AT_EXECUTION_STATE), "Invalid state map");
        const reconciliationResult = await reconciler_1.Reconciler.reconcile(ignitionModule, deploymentState, deploymentParameters, accounts, this._deploymentLoader, this._artifactResolver, defaultSender);
        ...
        if (this._hasBatchesToExecute(batches)) {
            ...
            const executionEngine = new execution_engine_1.ExecutionEngine(this._deploymentLoader, this._artifactResolver, this._executionStrategy, this._jsonRpcClient, this._executionEventListener, this._config.requiredConfirmations, this._config.timeBeforeBumpingFees, this._config.maxFeeBumps, this._config.blockPollingInterval);
    ...
    _emitDeploymentStartEvent(moduleId, deploymentDir, isResumed, maxFeeBumps) {
